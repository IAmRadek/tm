...
impl TrackingState {
    pub fn apply(&mut self, msg: DaemonMessage) {
        *self = match msg {
            DaemonMessage::Started { project, task, started_at }
            | DaemonMessage::Continued { project, task, started_at } => {
                TrackingState::Active { project, task, started_at }
            }
    ...
    pub fn label(&self) -> Option<String> {
        let TrackingState::Active { project, task, started_at } = self else {
            return None;
        };
